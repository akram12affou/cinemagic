...
function MovieCart({ movie, setQuery }: any) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 overflow-hidden">
      <div className="relative">
        ...
        >
          <div className="bg_color hover:-translate-y-0.5 transition ease-in-out rounded-md duration-700">
            {poster_path ? (
              <Link to={`/movie/${id}`}>
                <img
                  className="w-10 rounded-md img hover:sepia-[.5] cursor-pointer duration-700"
        ...
        <span
          className={
            "absolute text-white top-2 font-semibold tracking-wide text-sm p-1 rounded-r-lg md:text-base " +
            bg_color
          }
        >
          {vote()}
      ...
      <div className="flex items-center w-full justify-evenly  gap-3">
        <span
          className="text-white text-sm lg:text-base hover:tracking-wide cursor-pointer trans "
          title={title}
        >
          <Link to={`/movie/${id}`}>
