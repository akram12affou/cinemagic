    ...
    <div className="second_bg_color min-h-screen">
      ...
      >
        <div className="sm:w-10/12 w-11/12 mx-auto">
          <h2 className="font-semibold sm:text-2xl text-xl text-white decoration-1 pt-8">
            Watched List &#128151;
          </h2>
            ...
            {loading ? (
              ...
              <>
                {watchedList.map((movie: any) => {
                  return <MovieCart movie={movie} watchedList={watchedList} />;
                })}
              </>
            )}
            {!loading && watchedList.length == 0 && (
              <h1 className="text-white text-xl tracking-wide flex items-center gap-2">
                Your list is empty. Return home to build your watched collection{" "}
                <Link to="/">
                  <button className="bg-white text-black rounded-sm hover:scale-105 trans">
